...
# Supported MPU
# RZG2H, RZG2N, RZG2M, RZG2E, RZG2L, RZG2LC, RZG2UL, RZV2L, RZG3S, RZV2H

#----------------------------------------------
# Default Settings
#----------------------------------------------
TFA_DIR_DEFAULT=renesas-rz-tfa
UBOOT_DIR_DEFAULT=renesas-rz-uboot
FW_DIR_DEFAULT=rzg2_flash_writer
KERNEL_DIR_DEFAULT=renesas-rz-linux
...
if [ "$1" == "s" ] ; then
  ...
  "A  onekiwi-rzv2h-dev" "OneKiwi RZ/V2H DEV" \
  "B  onekiwi-rzv2h-8gb" "OneKiwi RZ/V2H 8GB" \
  "C  onekiwi-rzv2h-16gb" "OneKiwi RZ/V2H 16GB" \
  "D  onekiwi-rzv2l" "OneKiwi RZ/V2L" \
  ...
  "10  dev-rzt2h"   "Renesas DEV RZ/T2H" \
  "11  rzv2h-evk-ver1" "Renesas EVK RZ/V2H" \
	3>&1 1>&2 2>&3)
  RET=$?
  if [ $RET -eq 0 ] ; then
	...
	if [ "$?" == "0" ] ; then
      ...
      ;;
      10\ *) FW_BOARD=RZT2H_DEV ; MACHINE=dev-rzt2h ; MPU=RZT2H ; TFA_FIP=1 ;;
      9\ *) FW_BOARD=RZG3S_SMARC ; MACHINE=smarc-rzg3s ; MPU=RZG3S ; TFA_FIP=1 ;;
      11\ *) FW_BOARD=RZV2H_EVK ; MACHINE=rzv2h-evk-ver1 ; MPU=RZV2H ; TFA_FIP=1 ;;
      A\ *) FW_BOARD=ONEKIWI_RZV2H_DEV ; MACHINE=onekiwi-rzv2h-dev ; MPU=RZV2H ; TFA_FIP=1 ;;
      B\ *) FW_BOARD=ONEKIWI_RZV2H_8GB ; MACHINE=onekiwi-rzv2h-8gb ; MPU=RZV2H ; TFA_FIP=1 ;;
      C\ *) FW_BOARD=ONEKIWI_RZV2H_16GB ; MACHINE=onekiwi-rzv2h-16gb ; MPU=RZV2H ; TFA_FIP=1 ;;
      D\ *) FW_BOARD=ONEKIWI_RZV2L ; MACHINE=onekiwi-rzv2l ; MPU=RZV2L ; TFA_FIP=1 ; BOARD_VERSION="PMIC" ; FW_BOARD=RZV2L_SMARC_PMIC ;;
