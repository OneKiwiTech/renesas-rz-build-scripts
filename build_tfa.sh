...
if [ "$TFA_FIP" == "" ] ; then
  ...
  if [ ["$MACHINE" == "onekiwi-rzv2l" ] || ["$MACHINE" == "onekiwi-rzv2h-dev" ] || ["$MACHINE" == "onekiwi-rzv2h-8gb" ] || ["$MACHINE" == "onekiwi-rzv2h-16gb" ] || "$MACHINE" == "smarc-rzg2l" ] || [ "$MACHINE" == "smarc-rzg2lc" ] || [ "$MACHINE" == "smarc-rzv2l" ] || [ "$MACHINE" == "smarc-rzg2ul" ] || [ "$MACHINE" == "smarc-rzg3s" ] || ["$MACHINE" == "dev-rzt2h" ] || [ "$MACHINE" == "rzv2h-evk-ver1" ] ; then
    TFA_FIP=1
  else
    TFA_FIP=0
  fi
...
create_fip_and_copy() {
  ...
  if [ "$MACHINE" == "onekiwi-rzv2l" ] && [ "$BOARD_VERSION" == "PMIC" ] ; then
    EXTRA="_pmic"
  fi
...
case "$MACHINE" in
  ...
  "rzv2h-evk-ver1")
    ...
    ;;
  "onekiwi-rzv2h-dev")
    TFA_OPT="BOARD=onekiwi_dev"
    ;;
  "onekiwi-rzv2h-8gb")
    TFA_OPT="BOARD=onekiwi_8gb"
    ;;
  "onekiwi-rzv2h-16gb")
    TFA_OPT="BOARD=onekiwi_16gb"
    ;;
  "ek874")
